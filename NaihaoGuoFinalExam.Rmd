...
flights_delay <- tbl(my_db, sql("SELECT * FROM flights"))%>% collect()%>% mutate(canceled=is.na(arr_time))
...
weather.dep.fit <- lm(dep_delay ~ temp +  humid + wind_speed + visib, data = weather_delay_cancel)
...
weather.cancel.fit <- glm(canceled ~ temp +  humid + wind_speed + visib, data = weather_delay_cancel, family = binomial(link="logit"))
...
fig <- ggplot(flights_delay_df2 , aes(x= factor(hour),y=meandelay)) + geom_bar(stat="identity", fill = "skyblue")
fig
...
```{r}
time.cancel.fit.month <- glm(canceled ~ as.factor(month),data = flights_delay, family = binomial(link = "logit"))
...
```{r}
flights_delay$quarter <- ifelse(flights_delay$month %in% c(1,2,3),1, ifelse(flights_delay$month %in% c(4,5,6),2, ifelse(flights_delay$month %in% c(7,8,9),3,4)))
...
```{r}
time.cancel.fit.quarter <- glm(canceled ~ as.factor(quarter) ,data = flights_delay, family = binomial(link = "logit"))
...
fig <- ggplot(flights_delay_df2 , aes(x= factor(dayofweek),y=meandelay)) + geom_bar(stat="identity", fill = "skyblue")
fig
...
```{r}
time.cancel.fit.dayofweek <- glm(canceled ~ as.integer(dayofweek) ,data = flights_delay, family = binomial(link = "logit"))
...
```{r,warning=FALSE,eval=TRUE,message=FALSE}
flights_delay <- tbl(my_db, sql("SELECT dep_delay,arr_time, year, dest FROM flights")) %>% filter(year == 2013) %>% mutate(canceled = is.na(arr_time))
...
fig <- ggplot(tmp, aes(x= dest,y=meandelay)) + geom_bar(stat="identity", fill = "skyblue") + theme(axis.text.x=element_text(angle = 90))
fig
...
flights_delay <- tbl(my_db, sql("SELECT dep_delay,arr_time, year,tailnum FROM flights")) %>% filter(year == 2013) %>% mutate(canceled = is.na(arr_time))
...
tmp <- as.data.frame(tmp)
fig <- ggplot(tmp, aes(x= manufacturer,y=meandelay)) + geom_bar(stat="identity", fill = "skyblue") + theme(axis.text.x=element_text(angle = 90))
fig
flights_delay <- as.data.frame(flights_delay)
...
```{r,warning=FALSE,eval=TRUE,message=FALSE}
plane.cancel.fit <- glm(canceled ~  type + manufacturer + engines + engine, data= flights_delay, family = binomial(link = "logit"))
...
At last, it is expect takeoff delays at New York City airports in 2013 when Temperature  and Relative humidity are high, and the time may be some midnight in second half year and when the destinations are for example ALB and so on, if we know more about the planes, then if the plane is made by LAMBERT RICHARD and SIKORSKY and has a Reciprocating type engine. As for cancellations, it is with larger probability when it is weekend in second half year, one more interesting condition is when the plane have many engines.
